...
public static class InfiniBlazorMdComponentConverterFactory {
    public static IBlazorMdComponentConverter Create(IServiceProvider serviceProvider) {
        var config = serviceProvider.GetService<IMarkdownConfig>();
        ...
        return new BlazorMdComponentConverter {
            NodeToComponentMap =  config?.GetComponentRecords() ?? FrozenDictionary<Type, IMdComponentRecord>.Empty,
            SkippedComponentTypes = config?.GetSkippedBlazorComponentTypes() ?? FrozenSet<Type>.Empty,
            RenderUnknownComponents = config?.RenderUnknownBlazorComponents ?? false,
        };
    }
}
